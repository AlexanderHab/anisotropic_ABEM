 ...
 //   printf("g_neu_pone, p= %f, k=%f \n", p, k);
    ...
    if (k==0){
     ...
     //otherwise (k!=0) start recursion (formula (10))
         ...
         if(k-1 !=0){
             sol += (k-1)*(x*x+a*a)* g_neu_pone(p,y,x,a,k-2);
         }
 ...
 * 2 mal integrierte Kernfunktion int int 1 /|x-y| dy_2 dy_1  (ANALYTISCH)
 * 2 parameter l,k
 * Formula on Maischak p4. (bottom (15))
 */

double inline G_2_neu_alt(double p, double y1, double y2, double x1, double x2, 
        double a, double k, double l) {
      
    double sol = 0;

  //  printf("G_2_neu_alt: p= %f, k=%f, l=%f \n", p, k,l);
    

    //Zei 15
    sol += pow(y1,k-1)*g_neu_pone(p+1,y2,x2,sqrt( (y1-x1)*(y1-x1)+a*a ),l); 
    
    if(k+p != 0){
    
        sol += 2*x1*(k+p)*G_2_neu_alt(p,y1,y2,x1,x2,a,k-1,l);  
        
    }
    
    if( k-1 != 0){
    
        sol += (k-1)*( (x1*x1 + x2*x2 + a*a)*G_2_neu_alt(p,y1,y2,x1,x2,a,k-2,l)  
                + 2*x2*G_2_neu_alt(p,y1,y2,x1,x2,a,k-1,l+1)
                + 2*x2*G_2_neu_alt(p,y1,y2,x1,x2,a,k-2,l+2) );
        }
...
double inline G_2_neu(double p, double y1, double y2, double x1, double x2, 
    ...
    double sol = 0;
    double zz =0; 
 //  printf("G_2_neu: p= %f, k=%f, l=%f \n", p, k,l);
    
    if(p != -3./2){
        if( fabs(y1-x1) > pow(10,-13)){
...
//                 printf("G2(p-1,y1,y2,x1,x2,a,k,l))) sol normal 5: %f mit %f %f %f %f  %f %f %f %f \n", sol,p-1,y1,y2,x1,x2,a,k,l ); 
    ...
    //         printf("a= %f, x1 = %f, x2 = %f, y1= %f, y2=%f \n ",a,x1,x2,y1,y2); 
             ...
             sol = sol/(2*a) * acos( (-2)*(y1-x1)*(y1-x1)*(y2-x2)*(y2-x2)/ ( ((y1-x1)*(y1-x1)+a*a)*((y2-x2)*(y2-x2)+a*a)) +1);
//             if( isnan(sol))
//                 printf("G2 sol k=0 l=0: %f \n", sol); 
             }
        else if(k==0 && l==1){
            sol = asinh( (y1-x1) /  sqrt((y2-x2)*(y2-x2) +a*a) ) + x2*G_2_neu(p,y1,y2,x1,x2,a,0,0);
...
//                 printf("G_2_neu(p,y1,y2,x1,x2,a,0,l-2) sol k=0 l>1: %f mit %f %f %f %f  %f %f %f %f\n", sol,p,y1,y2,x1,x2,a,zz,l-2);
            ...
            sol = pow(-y1,k-1)*g_neu_pone(-1./2,y2,x2,sqrt((y1-x1)*(y1-x1) + a*a),l);
            if( isnan(sol))
...
double inline G_3_neu(double p, double x2, double y1, double y2, double x1_d1,
...
//    printf("G_3_neu: p= %f, l=%f, m=%f, n=%f \n", p,l,m,n);
	...
	sol += pow(y2,n+1)*G_2_neu(p,y1, x2, x1_d1,y2+d2,a,l,m);
    
    if(isnan(sol))
        printf(" first part in G_3: %f \n", G_2_neu(p,y1, x2, x1_d1,y2+d2,a,l,m));

    sol += pow(x2,l)*G_2_neu(p,y1,y2,x1_d1,x2-d2,a,m,n+1); 
    
    if(isnan(sol))
        printf(" second part in G_3: %f \n", G_2_neu(p,y1,y2,x1_d1,x2-d2,a,m,n+1));
            
    if(l!=0){
        sol += (-1)*l*G_3_neu(p,x2,y1,y2,x1_d1,d2,a,l-1,m,n+1);
        
        if(isnan(sol))
            printf(" third part in G_3: %f \n", G_3_neu(p,x2,y1,y2,x1_d1,d2,a,l-1,m,n+1));
    }
...
//          printf("F_par_pone: p= %f, k=%f,l=%f, m=%f, n=%f \n", p,k,l,m,n);
      ...
      sol += pow(x1,k)*G_3_neu(p,y2,y1,y2,x1-d1,d2,a,l,m+1,n);    //x1 is not sure!! typo on in the paper
