...
for j = 1:times
 ...
 fprintf("refine ok \n")
  ...
  b_pone = repmat([0.25;0.125;0.125;0.0625],24,1);
  ...
  for i = 1:length(typ)
    ...
    V_fine_pone = mex_build_V_pone(coordinates,elements,zeta{i},typ(i));
    build_time = toc;
    ...
    V_fine = mex_build_V(coordinates,elements,zeta{i},typ(i));
    build_time = toc;
    %testet auf fehlerhafte Eintraege (NaN +/-Inf)
    fprintf("finished max \n")
    ...
    %Loesung berechnen
       x_fine = V_fine\b;
       x_fine_pone = V_fine_pone\b_pone;
...
%     mu = hmin.*b.*sum((x_fine(f2s)'-repmat(x',4,1)).^2)'/4;
    ...
    xe_fine = b'*x_fine;
    xe_fine_pone = b_pone'*x_fine_pone;
    
    disp("difference in energy norm ");
